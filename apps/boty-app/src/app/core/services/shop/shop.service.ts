...
import { Firestore, collection, collectionData, doc, docData, setDoc, updateDoc, deleteDoc, getDoc } from '@angular/fire/firestore';
...
export class ShopService {
  ...
  /** Seed initial products into Firestore — only inserts if doc doesn't exist */
  async seedDatabase(): Promise<void> {
    ...
    console.log('Seed check complete — existing products were NOT overwritten.');
  }
  ...
  getProductById(id: string): Observable<Product> {
    return docData(doc(this.firestore, `products/${id}`), { idField: 'id' }).pipe(
      map(d => { if (!d) throw new Error('Product not found'); return d as Product; })
    );
  }
...
// ── Seed Data (solo se usa si Firestore está vacío) ─────────────────────────
const SEED_PRODUCTS: Product[] = [
  ...
  { id: '9', slug: 'bolso-mat-impermeable', name: 'Bolso Mat Impermeable', description: 'Múltiples bolsillos para accesorios.', details: 'Correa ajustable y tejido repelente al agua.', price: 28, category: 'Vida', stock: 25, images: ['https://images.unsplash.com/photo-1522845015757-50bce044e5da?auto=format&fit=crop&q=80&w=800'], thumbnail: 'https://images.unsplash.com/photo-1522845015757-50bce044e5da?auto=format&fit=crop&q=80&w=800', benefits: ['Práctico', 'Duradero'], materials: ['Nylon reciclado'] },
  ...
  { id: '15', slug: 'leggings-alta-compresion', name: 'Leggings Flow Sculpt', description: 'Alta compresión sin costuras.', details: 'Tejido técnico que abraza sin apretar. Opaco en sentadillas.', price: 45, category: 'Vida', stock: 22, images: ['https://images.unsplash.com/photo-1506126613408-eca07ce68773?auto=format&fit=crop&q=80&w=800'], thumbnail: 'https://images.unsplash.com/photo-1506126613408-eca07ce68773?auto=format&fit=crop&q=80&w=800', benefits: ['Transpirable', 'Efecto vientre plano'], materials: ['Nylon', 'Spandex'], isBestSeller: true },
  { id: '16', slug: 'top-sadhana', name: 'Top Sadhana Breath', description: 'Soporte medio, muy transpirable.', details: 'Escote en la espalda y copas removibles.', price: 30, category: 'Vida', stock: 28, images: ['https://images.unsplash.com/photo-1549643194-e699f7d46ffc?auto=format&fit=crop&q=80&w=800'], thumbnail: 'https://images.unsplash.com/photo-1549643194-e699f7d46ffc?auto=format&fit=crop&q=80&w=800', benefits: ['Libertad movimiento'], materials: ['Nylon', 'Elastano'] },
  { id: '17', slug: 'sudadera-mindful', name: 'Sudadera Mindful Eco', description: 'Algodón orgánico supersuave.', details: 'Corte oversize ideal para antes y después de tu práctica.', price: 55, category: 'Vida', stock: 15, images: ['https://images.unsplash.com/photo-1556821840-3a63f95609a7?auto=format&fit=crop&q=80&w=800'], thumbnail: 'https://images.unsplash.com/photo-1556821840-3a63f95609a7?auto=format&fit=crop&q=80&w=800', benefits: ['Tacto cálido'], materials: ['100% Algodón Peinado Orgánico'], isEcoFriendly: true },
